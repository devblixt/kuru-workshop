...
import {MeraPortfolioV2 as PortfolioPolicy} from "../src/MeraPortfolioV2.sol";
import {MeraPortfolio as PreviousMera, IBook} from "../src/MeraPortfolio.sol";

contract V2Token { uint8 public immutable decimals; constructor(uint8 d) { decimals = d; } }
contract V2Core {
...
contract MeraPortfolioV2Test is Test {
    PortfolioPolicy p; V2Core core; V2Book[3] books;
    PortfolioPolicy delegated; LegacyPolicy legacy; address mera=address(0xBEEF); uint256 constant KEY=12345; uint256 constant OWNER_KEY=67890; address owner=vm.addr(OWNER_KEY);
    PortfolioPolicy.Config c;
    function setUp() public {
        vm.warp(10 days + 12 hours);
        core=new V2Core(owner); address quote=address(new V2Token(6)); address[3] memory markets;
        uint8[3] memory ds=[uint8(6),18,8];
        for(uint256 i;i<3;++i) { books[i]=new V2Book(address(core),quote,address(new V2Token(ds[i]))); markets[i]=address(books[i]); }
        legacy=new LegacyPolicy(address(core),quote,markets); core.setLegacy(address(legacy));
        PreviousMera previous=new PreviousMera(address(core),quote,markets,address(legacy));
        p=new PortfolioPolicy(address(core),quote,markets,address(previous));
    ...
    function ownerAction(uint8 action) internal view returns(PortfolioPolicy.OwnerAction memory a){
        a=PortfolioPolicy.OwnerAction(1,action,mera,keccak256(abi.encode(c)),p.ownerNonces(1),core.epoch(),uint64(block.timestamp+120));
    }
    function ownerSig(PortfolioPolicy.OwnerAction memory a) internal view returns(bytes memory){(uint8 v,bytes32 r,bytes32 ss)=vm.sign(OWNER_KEY,p.hashOwnerAction(a));return abi.encodePacked(r,ss,v);}
    function testSponsoredConfigPauseReplayAndTampering() public {
        buy(0,10e6,5e6);PortfolioPolicy.OwnerAction memory a=ownerAction(0);bytes memory signature=ownerSig(a);
        p.executeOwnerAction(a,c,signature);assertEq(p.getUsage(1).bought,10e6);
        vm.expectRevert(PortfolioPolicy.Unauthorized.selector);p.executeOwnerAction(a,c,signature);
        a=ownerAction(0);signature=ownerSig(a);c.buyCap=500e6;
        vm.expectRevert(PortfolioPolicy.Unauthorized.selector);p.executeOwnerAction(a,c,signature);
        a=ownerAction(1);signature=ownerSig(a);p.executeOwnerAction(a,c,signature);assertTrue(p.getPolicy(1).paused);
    }
    function testOwnerSignatureExpiresAndAuthChangeInvalidates() public {
        PortfolioPolicy.OwnerAction memory a=ownerAction(0);bytes memory signature=ownerSig(a);core.revoke();core.reauthorize();
        vm.expectRevert(PortfolioPolicy.Unauthorized.selector);p.executeOwnerAction(a,c,signature);
        a=ownerAction(0);signature=ownerSig(a);vm.warp(block.timestamp+121);
        vm.expectRevert(PortfolioPolicy.Unauthorized.selector);p.executeOwnerAction(a,c,signature);
    }
}
