...
function App(){
 ...
 return <div className="shell">
  ...
  <section className="workspace"><div className="left-column">
   ...
   {!active?<button className="primary full" disabled={!!busy||!available||!marketReady||!Number(data.id)} onClick={()=>{setRefs(snap.markets.map((m:any)=>BigInt(m.reference)));setReview(true);}}>Review & enable manager <span>→</span></button>:<button className="stop full" disabled={!!busy} onClick={()=>void run('Stopping manager',async()=>{await api('/session/stop',{});await receipt(await (await ownerWallet()).writeContract({address:cfg.policy,abi:policyAbi,functionName:'pausePolicy',args:[Number(data.id)]}));})}>Stop manager & pause policy</button>}
   <div className="secondary-actions"><button disabled={!!busy||!available} onClick={()=>void run('Revoking permission',async()=>{await api('/session/stop',{});await receipt(await (await readyWallet()).account.revokeAccountSigner({account:address!,signer:cfg.policy}));})}>Revoke TRADE access</button><button disabled={!!busy} onClick={()=>void run('Withdrawing available USDC',async()=>{const k=await readyWallet();const balance=await readAvailableBalance(k,address!);if(!balance)throw new Error('No available USDC');await receipt(await k.account.withdraw({token:deployment.usdc,amount:balance}));})}>Withdraw available USDC ↗</button></div>
   </>}
  ...
  {review&&<div className="modal-backdrop"><div className="modal"><div className="section-title"><h2>Review trading permissions</h2><button onClick={()=>setReview(false)} disabled={!!busy}>×</button></div><p>Your wallet signs the configuration and grants the policy contract TRADE permission. Funds stay in AccountCore.</p><table><thead><tr><th>Asset</th><th>Reference</th><th>Average price band</th></tr></thead><tbody>{refs.map((r,i)=><tr key={i}><td>{deployment.markets[i].symbol}</td><td>{money(r)}</td><td>{money(r*97n/100n)} – {money(r*103n/100n)}</td></tr>)}</tbody></table><p>Buy cap {buyCap} USDC · Sell cap {sellCap} reference USDC · Per trade {tradeCap} USDC. Expires in at most 30 minutes.</p><p className="fine">Existing daily sell references and usage remain unchanged. These references are testnet market prices, not oracle valuations.</p><button className="primary full" disabled={!!busy} onClick={()=>void run('Configuring policy',async()=>{const k=await readyWallet();const expiry=(await publicClient!.getBlock()).timestamp+1800n;const config={manager:cfg.manager as Address,expiry,buyCap:parseUnits(buyCap,6),sellCap:parseUnits(sellCap,6),tradeCap:parseUnits(tradeCap,6),references:refs as [bigint,bigint,bigint],floors:refs.map(r=>r*97n/100n) as [bigint,bigint,bigint],ceilings:refs.map(r=>r*103n/100n) as [bigint,bigint,bigint]};await receipt(await (await ownerWallet()).writeContract({address:cfg.policy,abi:policyAbi,functionName:'configurePolicy',args:[Number(data.id),config]}));await receipt(await authorizeManager(k,address!,cfg.policy,expiry));await api('/sessions',{profile});setReview(false);})}>Confirm in wallet →</button></div></div>}
  <footer><span>Built on Kuru Spot · Monad testnet</span><span>Sessions expire automatically. You can pause or revoke access at any time.</span><a href="/downloads/kuru-workshop-starter.zip">Download workshop starter ↗</a></footer>
 </main></div>;
}
