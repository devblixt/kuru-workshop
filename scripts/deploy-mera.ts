...
const artifact=JSON.parse(readFileSync('contracts/out/MeraPortfolio.sol/MeraPortfolio.json','utf8'));
mkdirSync('artifacts',{recursive:true});
if(existsSync('artifacts/mera-deployment.json'))throw new Error('Deployment already recorded; refusing to redeploy');
let hash:Hex;
if(existsSync('artifacts/mera-deploy-pending.json')){
 const pending=JSON.parse(readFileSync('artifacts/mera-deploy-pending.json','utf8'));hash=pending.hash;
 ...
 const data=encodeDeployData({abi:artifact.abi,bytecode:artifact.bytecode.object,args:[deployment.core,deployment.usdc,deployment.markets.map(m=>m.address),config.policy]});
 ...
 writeFileSync('artifacts/mera-deploy-pending.json',stringify({hash,raw}),{mode:0o600});await wallet.sendRawTransaction({serializedTransaction:raw});
}
...
writeFileSync('artifacts/mera-deployment.json',JSON.stringify(record,null,2));
